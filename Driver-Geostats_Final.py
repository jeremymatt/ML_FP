...
#Load the paired head/tails of all parameters
paired_data = pd.read_excel('jan2017_13_pairs.xlsx')
...
#Add the predicted and the error variance values to the interpolation points
#dataframe
interp_points[primary] = Z
interp_points['{}|EV'.format(primary)] = EV

#Select the line to plot the predicted values and error variance and mask off
#the relevant data
line_to_plot = 'AD'
mask = interp_points['line']==line_to_plot
to_plot = interp_points.loc[mask,:]

#Fill out labels dict for plotting function
labels = {}
...
#y-variable to plot
plot_y = primary
#second y variable to plot
plot_y2 = '{}|EV'.format(primary)

#generate the plot along the line
ax1,ax2 = GSF.plot_model_1D(to_plot,plot_x,plot_y,plot_y2,labels)

#Plot a figure of the points and stations with the line of interested 
#highlighted
plt.figure()
...
#For each station:
#   1.  Leave station out of kriging and use other stations to interpolate to 
#   left-out station
#   2.  calculate error as predicted-actual
...
#Plot a figure showing the error for each station for the current timestep
plt.figure()
